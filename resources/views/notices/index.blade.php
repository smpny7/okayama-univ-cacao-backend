            ...
            <div class="flex flex-col w-full mb-6 lg:justify-between lg:flex-row md:mb-8">
                <div class="flex items-center mb-5 md:mb-6 group lg:max-w-xl">
                    ...
                    <h2 class="font-sans text-3xl font-bold leading-none tracking-tight text-gray-900 sm:text-4xl">
                        <span class="inline-block ml-4 tracking-widest">お知らせ追加 / 編集</span>
                    </h2>
                </div>
            </div>
            ...
            <div class="bg-white shadow-md rounded my-6">
                <table class="min-w-max w-full table-auto">
                    ...
                    <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                        <th class="hidden lg:table-cell py-3 px-6 text-left">送信者</th>
                        <th class="py-3 px-6 text-left">内容</th>
                        <th class="hidden lg:table-cell py-3 px-6 text-center">ステータス</th>
                        ...
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            <td class="hidden lg:table-cell py-3 px-6 text-left">
                                <div class="flex items-center">
                                    <img src="{{ $notice->sender_icon_url }}" class="h-5 w-5 rounded mr-3"
                                         alt="{{ $notice->sender_name }}">
                                    <span>{{ $notice->sender_name }}</span>
                                </div>
                            </td>
                            <td class="py-3 px-6 text-left">
                                <div class="flex items-center">
                                    <span>{{ mb_substr(strip_tags($notice->contents), 0, 30) }}</span>
                                </div>
                            </td>
                            <td class="hidden lg:table-cell py-3 px-6 text-center">
                                @if($notice->isPublished())
                                    <span
                                        class="bg-green-100 text-green-600 py-1 px-3 rounded-full text-xs">{{ $notice->released_at->format('Y/m/d H:i') . ' に公開済み' }}</span>
                                @else
                                    <span
                                        class="bg-pink-100 text-pink-600 py-1 px-3 rounded-full text-xs">{{ $notice->released_at->format('Y/m/d H:i') . ' に公開予定' }}</span>
                                @endif
                            </td>
                            <td class="py-3 px-6 text-center">
                                <div class="flex item-center justify-center">
                                    ...
                                    <form name="destroy" method="POST"
                                          action="{{ route('notices.destroy', ['notice' => $notice->id]) }}">
                                        @method('DELETE')
                                        @csrf
                                        <button type="submit"
                                                class="block w-4 mr-2 transform hover:text-themeColor hover:scale-110"
